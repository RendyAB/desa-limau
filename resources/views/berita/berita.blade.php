        ...
        @foreach ($berita as $item)
            <div class="col-12  col-lg-6">
                <div class="card p-1 my-3 w-100">
                    ...
                    @else
                        <img src="{{ asset('uploads/berita/default.jpg') }}" class="w-100 rounded"
                            style="height: 250px; object-fit: cover;" alt="Gambar tidak tersedia">
                    @endif
                    ...
                    <div class="vstack p-3 position-absolute w-100" style="z-index: 3">
                        ...
                        <a href="{{ route('berita.isiBerita', $item->id) }}"
                            ...
                            style="font-weight: bold">{{ Str::limit($item->judul,70)}}</a>
                        <div class="hstack align-items-center p-2 text-light gap-3">
                            <h6>
                                <a href="" class="link-underline text-light link-underline-opacity-0"><b
                                        class="bi bi-pen me-1"></b>
                                    {{ $item->penulis }} </a>
                            </h6>
                            <h6 class=""><b class="bi bi-calendar-date me-1"></b>
                                {{ $item->created_at->format(' d F Y') }}</h6>
                        </div>
                        <div class="">
                            <a href="{{ route('berita.isiBerita', $item->id) }}" class="btn btn-primary btn-sm">Read More
                                &gt;</a>
                        </div>
                    </div>
